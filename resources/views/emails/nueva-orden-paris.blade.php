...
<head>
    ...
    <style>
        ...
        .header { background: #E4002B; padding: 24px 32px; }
        .header h1 { margin: 0; font-size: 20px; color: #fff; }
        .header p { margin: 4px 0 0; font-size: 13px; color: #ffe0e6; }
        .body { padding: 24px 32px; }
        .orden { border: 1px solid #e5e5e5; border-radius: 6px; margin-bottom: 24px; overflow: hidden; }
        .orden-header { background: #f9f9f9; padding: 12px 16px; border-bottom: 1px solid #e5e5e5; }
        .orden-header strong { font-size: 14px; }
        .badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #fde2e2; color: #9b1c1c; }
...
<div class="wrapper">
    <div class="header">
        <h1>🔴 Nueva orden de Paris</h1>
        <p>{{ now()->format('d/m/Y H:i') }} — orden #{{ $orden['order_id'] }} sincronizada</p>
    </div>
    ...
    <div class="footer">
        Sistema GYC — sincronización automática de Paris Marketplace
    </div>
</div>
</body>
</html>
